...
class OrderItem(Base, TimestampMixin):
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "line_total >= 0",
            name="chk_order_items_line_total_non_negative",
        ),
        CheckConstraint(
            "discount_rate >= 0 AND discount_rate <= 1",
            name="chk_order_items_discount_rate_range",
        ),
        # Updated to support the DataCo dataset
        CheckConstraint(
            "profit_ratio >= -10 AND profit_ratio <= 10",
            name="chk_order_items_profit_ratio_range",
        ),
    )
